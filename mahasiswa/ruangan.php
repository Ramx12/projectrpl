...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['book'])) {
    ...
    // Jika lebih dari 1 hari, wajib upload dokumen
    if ($durasi_hari > 0) {
        if (!isset($_FILES['dokumen_pendukung']) || $_FILES['dokumen_pendukung']['error'] !== 0) {
            ...
            if (!in_array($ext, $allowed)) {
                ...
                $new_name = 'doc_ruangan_' . time() . '_' . $user_id . '.' . $ext;
    ...
    if (!$error) {
        ...
        if (strtotime($tanggal_mulai) > strtotime($tanggal_selesai)) {
            ...
            if ($result['total'] > 0) {
                $error = "Ruangan sudah dipinjam (disetujui) pada waktu tersebut.";
            } else {
                // Cek konflik dengan jadwal perkuliahan
                $conflict = false;
                $current_date = new DateTime($tanggal_mulai);
                $end_date = new DateTime($tanggal_selesai);
                
                while ($current_date <= $end_date) {
                    $hari = getHariIndonesia($current_date->format('N'));
                    ...
                    $jadwal_stmt->bind_param("isss", $ruangan_id, $hari, $jam_mulai, $jam_selesai);
                    $jadwal_stmt->execute();
                    $jadwal_result = $jadwal_stmt->get_result()->fetch_assoc();
                    
                    if ($jadwal_result['total'] > 0) {
                        $conflict = true;
                        $error = "Ruangan tidak dapat dipinjam pada " . $hari . ", " . $current_date->format('d/m/Y') . " karena ada jadwal perkuliahan.";
                        break;
                    }
                ...
                if (!$conflict) {
                    $insert_stmt = $conn->prepare("
                        ...
                        (user_id, ruangan_id, tanggal_mulai, tanggal_selesai, jam_mulai, jam_selesai, keperluan, dokumen_pendukung, status) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending')
                    ");
                    $insert_stmt->bind_param("iissssss", $user_id, $ruangan_id, $tanggal_mulai, $tanggal_selesai, $jam_mulai, $jam_selesai, $keperluan, $dokumen_pendukung);
                    
                    if ($insert_stmt->execute()) {
                        $success = "Berhasil diajukan! Menunggu persetujuan BOP.";
                    } else {
...
// Fungsi helper untuk konversi nomor hari ke nama hari Indonesia
function getHariIndonesia($day_number) {
    $hari = ['', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu', 'Minggu'];
    return $hari[$day_number];
}

// Ambil semua data ruangan (termasuk yang maintenance)
$ruangan_query = "SELECT * FROM ruangan ORDER BY status DESC, gedung, nama_ruangan";
            ...
            <div class="container">
                <div class="dashboard-nav">
                    <div class="dashboard-title">
                        <h1><i class="fas fa-door-open"></i> Peminjaman Ruangan</h1>
                ...
                <div class="dashboard-menu">
                    ...
                    <a href="ruangan.php" class="active"><i class="fas fa-door-open"></i> Pinjam Ruangan</a>
            ...
            <div class="container">
                ...
                <div class="room-grid">
                    <?php while ($ruangan = $ruangan_result->fetch_assoc()): ?>
                        <div class="room-card <?= $ruangan['status'] === 'maintenance' ? 'maintenance' : '' ?>" 
                             onclick="<?= $ruangan['status'] === 'tersedia' ? "openBookingModal(" . $ruangan['id'] . ", '" . htmlspecialchars($ruangan['nama_ruangan']) . "')" : '' ?>">
                            <img src="../assets/images/<?= htmlspecialchars($ruangan['gambar']) ?>" 
                                 alt="<?= htmlspecialchars($ruangan['nama_ruangan']) ?>">
                            <div class="room-card-content">
                                <div class="room-card-header">
                                    <div>
                                        <h3><?= htmlspecialchars($ruangan['nama_ruangan']) ?></h3>
                                    </div>
                                    <span class="room-badge <?= $ruangan['status'] === 'maintenance' ? 'maintenance' : '' ?>">
                                        <i class="fas fa-<?= $ruangan['status'] === 'tersedia' ? 'check-circle' : 'tools' ?>"></i> 
                                ...
                                <div class="room-info">
                                    <div class="room-info-item">
                                        ...
                                        <span><?= htmlspecialchars($ruangan['gedung']) ?></span>
                                    </div>
                                    <div class="room-info-item">
                                ...
                                <?php else: ?>
                                <button class="btn btn-primary" disabled style="width: 100%; margin-top: 10px;">
...
<div id="bookingModal" class="modal">
    <div class="modal-content">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group" id="dokumen_group">
                <label>
                    <i class="fas fa-file-upload"></i> Dokumen Pendukung
                    <span id="dokumen_required" style="color: #ef4444;"></span>
                </label>
                ...
                <div id="dokumen_required_notice">
                    <i class="fas fa-exclamation-triangle"></i>
                    Peminjaman lebih dari 1 hari wajib melampirkan dokumen pendukung
                    (surat izin, proposal kegiatan, dll)
                </div>
            </div>
...
<script>
    function openBookingModal(id, nama) {
        ...
        document.getElementById('modalTitle').innerHTML =
            '<i class="fas fa-door-open"></i> Pinjam ' + nama;
    }
    ...
    function closeBookingModal() {
        ...
        document.querySelector('.file-name').textContent =
            'Pilih file (PDF, DOC, DOCX, JPG, PNG)';
        document.getElementById('dokumen_required_notice').style.display = 'none';
        document.getElementById('dokumen_required').textContent = '';
    }
    ...
    function checkDuration() {
        ...
        if (startDate && endDate) {
            ...
            if (diffDays > 0) {
                document.getElementById('dokumen_required').textContent = '*';
                document.getElementById('dokumen_required_notice').style.display = 'block';
                document.getElementById('dokumen_pendukung').required = true;
            } else {
                document.getElementById('dokumen_required').textContent = '';
                document.getElementById('dokumen_required_notice').style.display = 'none';
                document.getElementById('dokumen_pendukung').required = false;
            }
        }
    }

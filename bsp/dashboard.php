...
if (!isset($_SESSION['role']) || $_SESSION['role'] !== 'staff_bsp') {
...
if (!isset($_SESSION['staff_bsp_id'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM staff_bsp WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['approve'])) {
        ...
        $stmt = $conn->prepare("UPDATE peminjaman_fasilitas SET status = 'disetujui', disetujui_oleh = ?, nama_penyetuju = ? WHERE id = ?");
        $stmt->bind_param("isi", $staff_id, $staff_nama, $peminjaman_id);
        $stmt->execute();
        $success = "Peminjaman berhasil disetujui!";
    } elseif (isset($_POST['reject'])) {
        $peminjaman_id = (int)$_POST['peminjaman_id'];
        $catatan = htmlspecialchars($_POST['catatan'], ENT_QUOTES, 'UTF-8');
        $stmt = $conn->prepare("UPDATE peminjaman_fasilitas SET status = 'ditolak', catatan = ? WHERE id = ?");
...
// Get statistics
$total_fasilitas = $conn->query("SELECT COUNT(*) as total FROM fasilitas")->fetch_assoc()['total'];
...
$approved_query = "
    ...
    JOIN fasilitas f ON pf.fasilitas_id = f.id
    JOIN users u ON pf.user_id = u.id
    WHERE pf.status = 'disetujui'
    ORDER BY pf.tanggal_mulai DESC
    LIMIT 10
";
...
<head>
    ...
    <title>Dashboard BSP - UMB Booking System</title>
                ...
                <div class="dashboard-nav">
                    <div class="nav-brand">
                        ...
                        <div class="dashboard-title">
                            <h1><i class="fas fa-home"></i> Dashboard Staff BSP </h1>
                            <p>Biro Sarana Prasaran - Pengelola Fasilitas UMB</p>
                        </div>
                    </div>
                    <div class="user-info">
            ...
            <div class="container">
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <a href="riwayatpeminjaman.php" class="btn btn-outline">Lihat Semua</a>
                    </div>
